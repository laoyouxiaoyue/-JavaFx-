...
public class ChangeReward {
    ...
    public  void initialize()
    {
        ...
        Code.getItems().add("0--校特等奖学金");
        Code.getItems().add("1--校一等奖学金");
        Code.getItems().add("2--校二等奖学金");
        Code.getItems().add("3--校三等奖学金");
        Code.getItems().add("4--系一等奖学金");
        Code.getItems().add("5--系二等奖学金");
        Code.getItems().add("6--系三等奖学金");
    ...
    void OnSubmit(ActionEvent event) throws SQLException {
        ...
        String sql="update reward set 记录号='"+sno+"',学号='"+ssno+"',级别代码='"+code+"',记录时间='"+recordTimeValue+"',描述='"+miaoshu +"' where 记录号='"+sno+"'";
        ...
        check="select * from reward_levels where 代码='"+code+"'";
        rs=stat.executeQuery(check);
        if(!rs.next())
        {
    ...
    public ChangeReward(Connection conn, Statement stat, ObservableList observableList) {
        this.conn = conn;
        this.stat = stat;
