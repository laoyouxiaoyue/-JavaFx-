...
public class AddPunishment {
    ...
    Connection conn;
    Statement stat;
    public AddPunishment(Connection conn, Statement stat) {
        this.conn = conn;
        this.stat = stat;
    }
    @FXML
    ...
    void OnSubmit(ActionEvent event) throws SQLException {
        ...
        String sql = "insert into punishment (记录号, 学号, 级别代码, 记录时间,是否生效,描述) values (?,?,?,?,?,?)";
        ...
        check="select * from punishment where 记录号='"+sno+"'";
        rs=stat.executeQuery(check);
        if(rs.next())
        {
        ...
        preStmt.setString(5,sx);
        preStmt.setString(6, miaoshu);
