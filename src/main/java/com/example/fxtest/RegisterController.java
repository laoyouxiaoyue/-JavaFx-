...
public class RegisterController {
    ...
    public RegisterController(Connection conn,Statement stat) {
        this.conn=conn;
    ...
    public void initialize()
    {
        ...
        Birth.getEditor().setStyle("-fx-background-color: rgb(41,47,76) rgb(41,47,76);-fx-text-fill:white");
       ...
       if (ok.next()) {
        ...
        if(!KaMi.getText().equals(""))
        {
            ...
            if(rs.next())
            {
                if(rs.getInt(1)==1)
                {
                        preStmt.setInt(6,rs.getInt(2));
                        String sql2="update kami set 用户名='"+yonghuming+"',时间='"+Birth.getValue().toString()+"',有效=0 where ID='"+KaMi.getText()+"'";
                        stat.execute(sql2);
                }
                else {
